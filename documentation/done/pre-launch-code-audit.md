...
> - ✅ **Sektion 5 (Duplikation #64–#71): alle 8 entkoppelt** (eigener Commit). Neue Single-Sources: `rootFinder.ROOT_FILE_CANDIDATES`/`findRootFileIn` (6 Stellen), `fileManager.applySpellcheckLanguage` (2), `gitManager.ensureGitignore` (3, jetzt inkl. Template), **`shared/refLabels.ts`** (Referenz-vs-Zitat-Vokabular + Label-Typ — 4 Stellen, vereinheitlicht auf die Editor-Semantik: Doppelpunkt oder Exact-Match, `fig-intro` ist jetzt überall Zitat und `eqn:`/`tab:`/`thm:` etc. auch in DOCX/HTML eine Referenz), `exportContext.matchBracket` (gehärtet auf echte Escape-State-Machine — der alte Lookbehind scheiterte an `\\"` — htmlSerializer-Kopie gelöscht), App.svelte `selectionOrWord`/`toProjectRelPath`/`fetchProjectDir` (Kommentar- + Design-Pin-Pfad + refreshCommentMarks).
> - ⏳ Offen: Sektion **6 (Plausible, 13 Stück — je Laufzeit-Repro vor Fix nötig)**. Bekannte akzeptierte Limitation aus dem Review: die neue PM-basierte Suche findet keinen Text mehr in Node-**Attrs** (Opener-Titel, Captions, Raw-Block-Bodies) — Attr-Treffer wären nicht ersetzbar; als Follow-up notiert.
