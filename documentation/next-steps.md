...
- **🆕 „Für den Druck exportieren" (Print-Export) — voller Plan in [print-export-plan.md](print-export-plan.md).** Hebt Penwright vom Bildschirm-PDF auf **druckerei-tauglich** für hochwertige Magazine/Broschüren (konkreter Anlass: Garten-/Landschaftsbau-Magazin einer Agentur-Kundin). **In-App, rein Typst-intern (kein Engine-Warten):** Beschnitt (Bleed) + Schnittmarken + Innen-/Außenstege (Bundzuwachs) + dpi-Preflight → eigener Export-Transform (temp `style-print.typ`, kein safe-apply nötig). **Schema:** `StyleLayout` um `bleed`/`cropMarks`/`facingPages`/`binding`; **Generator** (`styleParser.ts`): oversized `width`/`height` statt `paper:` + Papiermaß-Tabelle + `crop-marks()`-Helper im `foreground`; **Elemente** (`designElements.ts`): Full-Bleed-Snippets bleed-bewusst via exportiertem `style-bleed`. **Bewusst NICHT in-App:** CMYK/PDF-X mit gesetzten Boxen — Typst kann weder ICC-Profil einbetten ([#3143](https://github.com/typst/typst/issues/3143)) noch TrimBox/BleedBox setzen (nur MediaBox), d. h. die **gezeichneten Schnittmarken SIND die Trim-Definition**; CMYK bleibt Nachschritt (Druckerei / Acrobat / Ghostscript-AGPL-Caveat). **Stretch:** Doppelseiten-Bilder (`spread-image`, Seitenparität + Bundverlust beachten) + **2-up-Doppelseiten-Vorschau** in `PdfPreviewPanel` (eigenständiger Magazin-Quick-Win). Aufwand MVP ≈ 1 fokussierte Session; Reihenfolge + Test-Checkliste im Plan.
