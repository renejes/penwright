...
> Audit-Datum: 2026-04-17 | Letzte Aktualisierung: 2026-06-29 (Session 36: Web-Export Phase 0/A/B) | App-Version: **0.9.0** (package.json + Doku synchron)
>
...
### 🔑 Nächste Session: Web-Export Phase C (Keystone) — VOR dem Launch (neuer Chat)

**Entscheidung 2026-06-29 (Session 36):** Ein **HTML-/Web-Export** („Editorial Web Pack") kommt **vor** dem Launch und wird die Launch-Story „**Print UND Web aus einer Quelle**" (eigene Artikel, Kunden-Artikel, Blog-Design — Markt-Lücke: niemand macht „ein strukturiertes Manuskript → design-treuer Print + responsives Web aus einer Quelle"). Volle Recherche + Markt-Einordnung + Korrekturen am Ur-Plan + de-riskte Phaseneinteilung: **[web-export-feasibility-and-plan.md](web-export-feasibility-and-plan.md)** (PFLICHTLEKTÜRE).

**Stand:** **Phase 0 (2 Pre-Launch-Bugfixes) + Phase A (Foundation `htmlSerializer.ts`, `@tiptap/static-renderer` gepinnt) + Phase B (design-treuer Slice: `styleToCss.ts`, Drop-Cap + Callout, agnostischer Bundle-Writer `webExport.ts`, File ▸ „Export to Web (HTML)…")** sind gebaut + verifiziert (tsc + build + 52er-Smoke-Test + 37/37-Round-Trip, am echten LANGSAM validiert) — auf Branch `feat/web-export`, committet, **nicht gemergt/gepusht**.

**Der neue Chat = Phase C (der Keystone):** die tragenden Magazin-Makros (`opener`/`pull`/`frage`/`notiz`/`bildtafel`/`randnotiz` + `#columns`) zu **echten benannten AST-Nodes** machen → sie tragen dann in **einem** Schritt nach **PDF + DOCX + HTML** (kein „Text mit Löchern" mehr) und werden im Editor echt-WYSIWYG editierbar. ~1,5–2,5 Wochen, höchstes Regressionsrisiko (Editor-Deserializer/Serializer/Schema). **Exakte Aufgabenliste, Reihenfolge, Hard Gates + offene Entscheidungen:** `handover.md` → „Nächste Session — Fokus: 🔑 PHASE C" + Plan-Doc §1.3 / §Phase C / §6 / §7. Danach **Phase D + E** (Web-Export fertig), **dann** der Release-Sprint unten.

### 🚀 Danach: Release-Sprint (nach dem Web-Pack C+D+E)
