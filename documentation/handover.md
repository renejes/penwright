...
> **Stand:** 2026-06-08, Ende der UI-Polish-/Bugfix-Session (Session 31). **Code + Doku uncommittet im Working-Tree auf `main`** (User committet auf Ansage).
> **Zuletzt erledigt:** Session 31 = (1) **StartScreen aufgeräumt** (KI-Anbindung-Block raus, Logo zentriert via getrimmter SVG-viewBox, Untertitel → „Typst, visuell geschrieben" / „Typst, written visually", der oben-rechts-EN/DE-Schalter raus — Statusleisten-Toggle reicht); (2) **zwei echte Bugs gefixt:** `app:checkTypst` lief durch einen Runtime-`require('./typstPath')` im gebündelten Main *immer* auf „Typst nicht gefunden" (gleicher Bug in `crashReporter` mit-gefixt) → jetzt statische Imports; „Version speichern" blieb bei git-losen Projekten (z. B. LANGSAM) ausgegraut — die **erste Version legt das Repo jetzt an**; (3) **MCP Apps** + **Design-Vorher/Nachher** ausführlich evaluiert → **beide bewusst verworfen** (Begründung unten). Siehe **„Status Session 31"** unten.
> **Davor:** Session 30 = Politur (0 Build-Warnungen, Onboarding aufs Look-Modell, „Verlauf & Wiederherstellen"-Hub); Session 29 = **Fokus-Schnitt** (CLI/Terminal/Modi raus — §1d); Session 28 = volles i18n + ＋-Einfügen-Menü + Vorschau (Auto/Manuell + folgt Kapitel).
