...
> **Nächste Session — Fokus:** **Launch-Vorbereitung, keine neuen Features** — die App ist inhaltlich fertig. (1) `penwright.online` registrieren (Blocker), (2) manuelle Smoke-Tests, v. a. der **„von außen geöffnetes Projekt"-Pfad** (s. u.), (3) Showcase-Projekte + Windows-Verifikation. Siehe **„Nächste Session — Fokus"** unten.
...
- **🆕 „Für den Druck exportieren" (Print-Export) — geplant (post-launch), voller Plan in [print-export-plan.md](print-export-plan.md).** Macht Penwright **druckerei-tauglich** für hochwertige Magazine/Broschüren (Anlass: Garten-/Landschaftsbau-Magazin einer Agentur-Kundin). **Recherche-Kern (2026-06-13):** das gestaltete PDF ist heute reines **RGB-Bildschirm-PDF** ohne Bleed/Schnittmarken/CMYK/PDF-X (Audit: `StyleLayout` hat nur eine uniforme `margin`, `buildTypstCompileArgs` setzt keine Druck-Flags, Full-Bleed-Elemente gehen von Seite=Endformat aus). **Plan:** in-App **Bleed + Schnittmarken + Innen-/Außenstege + dpi-Preflight** (rein Typst-intern, eigener Export-Transform via temp `style-print.typ`, **kein** safe-apply nötig); **bewusst draußen:** CMYK/PDF-X — Typst kann kein ICC-Profil einbetten ([typst#3143](https://github.com/typst/typst/issues/3143)) und keine TrimBox/BleedBox setzen (nur MediaBox), die **gezeichneten Marken sind die Trim-Definition**, CMYK = Nachschritt (Druckerei/Acrobat/Ghostscript-AGPL-Caveat). **Stretch:** Doppelseiten-Bild (`spread-image`, Parität + Bundverlust) + **2-up-Vorschau** in `PdfPreviewPanel`. MVP ≈ 1 Session. **Nicht launch-blocking** — die App bleibt inhaltlich fertig; dies ist ein bewusst geplantes v1.1-Feature für den Print-/Agentur-Use-Case.
