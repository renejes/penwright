...
**✅ Phase D status (2026-06-30, branch `feat/web-export`, Session 38).** Done + verified end-to-end; `tsc` + `electron-vite build` + `esbuild.mcp` clean; **4 suites green: roundtrip 76/76, html-export 146/146, docx 19/19, compile-stability 30/30 (real LANGSAM, untouched).** The web export is now content-complete for academic **and** magazine documents.
- **Shared analysis extracted → [exportContext.ts](../src/shared/exportContext.ts) (the single source of truth, plan §Phase D recommendation).** The format-agnostic primitives (`classifyRawBlock` + helpers, `splitHeadingLabel`, `refWords`, `citationInner`, bib formatting, `NUMERIC_BIB_STYLES`, `collectCitekeysInOrder`, `renderEqNumber`, `mathSnippet`/`svgSnippet`, `RawDesc`/`RefTarget`) were **moved out of docxSerializer** (verbatim, behaviour-preserving — docx test still 19/19) and a new pure `buildExportModel(doc, typstContent)` does the cross-ref-numbering / citation-style pre-pass for BOTH formats. `parseBibDirective` reads the `#bibliography(…)` args.
- **Architecture invariant honoured:** `serializeHtml` (and all of `htmlSerializer.ts` + `exportContext.ts`) stays **sync + pure** (no fs/async/Electron). The async Typst-run + bib-load live in **main** (`importExport.buildHtmlExportContext` + `createTypstSvgRenderer` = math → **SVG**, not KaTeX, C8) and are handed in as `opts.context: HtmlExportContext` (resolved labels / citation mode / bibEntries / math-SVG map / lang). `buildWebBundle` threads it through.
- **What now renders (was placeholder/raw):** **cross-references** (`@fig:x`→"Figure 1", `@sec:y`→"Section 2.1", `@eq:z`→"(1)", linked to the in-page `id`), **citations** (adjacent `@a @b` collapse → `(Bender et al., 2021; …)` author-year / `[1, 2]` numeric, each linked to `#cite-KEY`), **footnotes** (numbered `<sup>` + an endnotes `<section>` with ↩ backlinks), **bibliography** (`<section class="pw-bibliography">`, APA-shaped, anchored by citekey, alphabetical or numeric), **display math** (inline `<svg>` from the bundled Typst + `(n)` eqno), and the leftover raw blocks (`#figure` image/table → numbered `<figure>`/`<table>`, `#quote` → `<blockquote>`, design-text → styled `<p>`). The numbered constructs share ONE numbering pass with DOCX. New `.pw-math`/`.pw-footnotes`/`.pw-bibliography`/`.pw-figure` CSS in `styleToCss`.
- **Asset fix:** `makeAssetRewriter` now climbs a leading `../` from the root (chapter figures say `image("../assets/x.png")`) while keeping the in-root traversal guard — so magazine figures actually copy into the bundle.
- **Real proof:** the **sample project** (academic — 2 real math SVGs, 11 citations, 7 cross-refs incl. resolved `@sec`, footnote, 5 bib entries, figures+tables) **and the real `~/Desktop/LANGSAM` magazine** (5 openers, 3 pull-quotes, 2 columns, the interview grid) both export with **0 leftover placeholders** ([scripts/web-export-proof.mts](../scripts/web-export-proof.mts), uses the bundled Typst).
- **Adversarial multi-agent review (5 dimensions × per-finding skeptic verification): 7 real findings, all fixed + regression-tested.** Highest-value: (a) the pre-pass walked only top-level nodes while the renderer descends, so a figure/equation **nested in `#columns`/`#notiz`/`#bildtafel`** (the B1-fix child nodes) desynced caption numbers from cross-refs and dropped nested-math SVG → `buildExportModel` now does a depth-first walk matching the render; (b) **`@sec:` cross-refs never resolved** because the deserializer moved the heading label to `attrs.label` (B2) but the numbering pass read it from the stripped text → now prefers `attrs.label` (fixed in BOTH html + docx — a pre-existing DOCX bug too); (c) the inlined math **SVG was an XSS sink** (`$ #link("javascript:…")[x] $` → live `<a href="javascript:…">`) → `sanitizeSvg` now drops `<foreignObject>`/`on*=`/dangerous `href` while keeping `#glyph` `<use>` refs. Plus: footnote-only design chunks kept (no orphan endnote), `parseBibDirective` title survives a `)` in the path.
- **Deferred to Phase E (was a stretch D5):** section-overlay CSS (`.pw-section-<id>` from `style.sections`) + chapter-container wiring (needs the section-marker boundary detection in the merged doc) — low value, fragile; folded into E.
