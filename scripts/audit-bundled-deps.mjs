...
// ── Walk the font bundle ───────────────────────────────────────────────
// Fonts live one-directory-deep under resources/fonts/<Family>/. Each
// family ships an OFL.txt sibling — that's the only file we audit
// (the .ttf binaries themselves don't carry license metadata). Metadata
// for the in-app dialog comes from resources/fonts/manifest.json,
// which `scripts/fetch-typst-fonts.mjs` writes alongside the binaries.
...
if (existsSync(FONTS_DIR)) {
  ...
  if (existsSync(manifestPath)) {
    try {
      fontManifest = JSON.parse(readFileSync(manifestPath, 'utf-8'));
    } catch (err) {
      errors.push(`fonts/manifest.json: parse failed (${err.message})`);
    }
  }

  for (const familyName of readdirSync(FONTS_DIR).sort()) {
    const familyDir = join(FONTS_DIR, familyName);
    if (!statSync(familyDir).isDirectory()) continue;

    const licensePath = join(familyDir, 'OFL.txt');
    if (!existsSync(licensePath)) {
      errors.push(`fonts/${familyName}: missing OFL.txt`);
      continue;
    }
    ...
    const classified = classifyLicense(licenseText);
    if (!classified) {
      errors.push(`fonts/${familyName}: OFL.txt could not be classified. Update LICENSE_DEFINITIONS or replace.`);
      continue;
    }
    if (!classified.bundleable) {
      errors.push(`fonts/${familyName}: license ${classified.id} is on the deny-list.`);
      continue;
    }

    // Find the manifest entry — match by directory name with whitespace
    // stripped, since the fetcher uses that pattern for the on-disk slug.
    const manifestEntry = (fontManifest?.fonts ?? []).find(
      f => f.family.replace(/\s+/g, '') === familyName,
    );

    // Count the actual font files so we can show "Inter (4 files)" in the
    // markdown without re-reading the manifest at render time.
    const fontFiles = readdirSync(familyDir).filter(
      f => f.endsWith('.ttf') || f.endsWith('.otf'),
    );

    fontFindings.push({
      family: manifestEntry?.family ?? familyName,
      slug: familyName,
      ...
      licensePath: `resources/fonts/${familyName}/OFL.txt`,
    });
  }
}
...
const markdown = [
  ...
  '',
  `Total Typst packages: **${findings.length}**. Total bundled font families: **${fontFindings.length}**.`,
  '',
  '## Typst Packages',
  '',
  ...
  '## Bundled Fonts',
  '',
  fontFindings.length === 0 ? '_No fonts bundled (run `node scripts/fetch-typst-fonts.mjs` to populate)._' : null,
  fontFindings.length === 0 ? '' : '| Family | Category | License | Files | Source |',
  fontFindings.length === 0 ? null : '|---|---|---|---|---|',
  ...fontFindings.map(f =>
    `| ${f.family} | ${f.category} | ${f.license} | ${f.fileCount} | ${f.source ? `[link](${f.source})` : '—'} |`,
  ),
  '',
  ...
  fontFindings.length === 0 ? null : '## Per-Font License Texts',
  fontFindings.length === 0 ? null : '',
  ...fontFindings.flatMap(f => {
    const text = readFileSync(join(ROOT, f.licensePath), 'utf-8').trim();
    return [
      `### ${f.family}`,
      '',
...
//    Schema: { packages: [...], fonts: [...] }. The dialog renders one
//    section per top-level key.
const jsonOut = {
  generatedAt: new Date().toISOString(),
  totalPackages: findings.length,
  totalFonts: fontFindings.length,
...
console.log(`[audit] ✓ ${findings.length} packages + ${fontFindings.length} font families, all licenses bundleable`);
for (const [lic, n] of Object.entries(byLicense).sort()) {
  console.log(`  - packages ${lic}: ${n}`);
}
const fontByLicense = fontFindings.reduce((acc, f) => {
  acc[f.license] = (acc[f.license] || 0) + 1;
  return acc;
}, {});
for (const [lic, n] of Object.entries(fontByLicense).sort()) {
  console.log(`  - fonts ${lic}: ${n}`);
}
