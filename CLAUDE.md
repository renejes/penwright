...
Code used by both main and renderer: settings parser, bib parser, DOCX serializer, style/project templates, document merge/split, markdown importer. Also `skillTemplates.ts` — three Claude-Code-skill string constants (`TYPST_SKILL`, `VSWRITE_SKILL`, `RESEARCH_SKILL`) with tilde-fenced code blocks (avoids backtick escapes in TS template literals); `projectManager.ensureClaudeSkills` deploys these to `<project>/.claude/skills/<name>/SKILL.md` per-file. Import via `@shared` alias.
...
- **`src/cli/` and `src/mcp/`** exist from the VS Code extension; CLI is unused. The MCP server runs as a standalone Node.js process. Since Session 16 it exposes 43 typed tools across 12 categories (Project / Document / Settings / Chapters / Bibliography / Cross-Refs+Footnotes / Comments / Versions / Discovery / Export / Import+Assets / Git low-level) plus three skill prompts. `vswrite_compile` is a verifier — PDF temp-file then unlinked; `vswrite_export_pdf` and `vswrite_export_docx` own artefact writing and route through `resolveInsideProject`. Anchor-based tools (`add_comment` / `insert_reference` / `add_footnote` / `add_image`) take an `afterText`/`anchor` plus an optional 1-based `occurrence` parameter so the agent never computes offsets. Full reference: [documentation/mcp-server.md](documentation/mcp-server.md).
